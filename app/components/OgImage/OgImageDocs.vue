...
<template>
  <div class="size-full flex flex-col justify-center">
    ...
    <div class="pl-[100px]">
      <p v-if="headline" class="uppercase text-2xl text-sucess mb-4 font-semibold">
        {{ headline }}
      </p>
      <h1 v-if="title" class="m-0 text-7xl font-semibold mb-4 text-default flex items-center">
        <span>{{ title }}</span>
      </h1>
      <p v-if="description" class="text-3xl text-muted leading-tight w-[700px]">
        {{ description }}
      </p>
    </div>
  </div>
